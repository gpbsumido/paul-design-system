...
/**
 * A wall of image tiles drifting in 3D — each column scrolls at its own speed,
 * the plane tilts toward the pointer, and the tile under the cursor (or focus)
 * lifts forward and brightens. Ported from ReactBits and reimplemented on the
 * tokens; the drift and parallax run on a single rAF loop with no dependency.
 * Under reduced motion the columns hold still and only the focus lift remains.
 */
...
export function DriftWall({
  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number) => {
    ...
    const commonProps = {
      className: cx('drift-wall__tile', activeId === id && 'is-active'),
      'data-tile-id': id,
      'data-col': colIndex,
  ...
  return (
    <div
      ref={containerRef}
      className={cx('drift-wall', reduced && 'drift-wall--reduced', className)}
      style={cssVars}
