...
- **BotanicalText, reinterpreted from OriginKit.** Text grown from tiny flowers and leaves that sway. The original renders instanced sprites in Three.js; this samples the text on the built-in 2D canvas and draws little blooms at each point — **no WebGL, no dependency**. Blooms near the pointer open larger, as if reaching toward it. A labelled `role="img"`; under reduced motion the sway and pointer-reaction stop and the blooms are drawn at rest.
- **FolderFloat, reinterpreted from ReactBits.** A folder whose contents float out above it, fanned and gently bobbing. The original runs a matter-js physics simulation; this reinterprets the feel with a CSS fan and a per-chip bob — **no physics engine, no dependency**. A labelled `role="group"`; chips are real links when given an `href`, and the bob stops under reduced motion.
- **RefineFrame, reinterpreted from ReactBits.** A frame whose content resolves from blurred and desaturated to sharp as its status advances (`queued → generating → refining → complete`, plus `error`) — the preview an image model streams back. The original pixelates on a canvas; this drives CSS `filter`/scale/opacity per stage, so there's **no canvas and no icon dependency** (the status icons are inline SVG). A glint sweeps while it works and a `role="status"` pill announces the stage; under reduced motion the sweep drops and the spinner pulses instead of spinning.
- **PathGallery, reinterpreted from OriginKit.** Images gliding along a curved closed path, spaced evenly and looping. Built with CSS motion path (`offset-path`) — **no dependency**. Items are real links, the strip is a labelled `role="group"`, the dashed track line is toggleable (`showPath`), and under reduced motion the travel stops and the images rest along the path.
...
- **SmoothScrollSlider, reinterpreted from OriginKit.** A horizontal rail of cards that grow toward the centre and coast after a flick — a target offset and a rendered offset joined by a frame-rate-independent lerp give the momentum, and each card scales by its distance from the centre. Wheel, trackpad and drag all drive it, with an optional infinite loop. A labelled `role="group"` with the cards as real links; under reduced motion the coast is removed (the rail tracks input directly) while the spatial centre-scaling stays. No runtime dependency.
- **Two OriginKit components, reinterpreted on the tokens.** **HoverImageReveal** — a vertical text menu (real links) that reveals a per-row image in a window trailing the cursor, the active image fading in over the others. **LinkPreview** — an inline link that raises a floating thumbnail on hover which leans toward the pointer (custom-image mode, no external screenshot API); the card is decorative and the real `<a>` carries the link. Both are keyboard-operable, axe-clean, and reduced-motion aware (the cursor-follow and lean drop to a plain fade). No runtime dependency.
- **DriftWall, ported from ReactBits.** A 3D wall of image tiles — each column drifts at its own speed on a single rAF loop, the plane tilts toward the pointer, and the hovered or focused tile lifts forward and brightens. Tiles are real focusable buttons (or links when given an `href`), the container is a labelled `role="group"`, and under reduced motion the columns hold still while the focus lift stays. No runtime dependency.
- **LatticeLoader, ported from ReactBits.** A grid of cells that light in a travelling pattern (orbit/sweep/dots/ripple/… on a 3×3 or 4×4 grid) while working, resolving into a check or cross when it finishes, with a live timer. `role="status"` with an off-screen spoken update, so the state reaches assistive tech regardless of the animation; under reduced motion the pulse flattens to a gentle breathe and the mark stops transforming. No runtime dependency — the original's pattern engine and CSS, reimplemented on the tokens.
- **Three interaction primitives from the ReactBits set, reinterpreted on the tokens.** **TextLoop** cycles a list of phrases in place, each sliding up as the next arrives (only the active phrase reaches assistive tech). **SquishSwitch** is a `role="switch"` whose thumb springs across and compresses on press. **RubberSegment** is a `role="radiogroup"` whose indicator rubber-bands between segments, overshooting and settling. All keyboard-operable, axe-clean, and reduced-motion aware (the slide/spring/elastic travel drop to a plain state change). Each ships a `@paul-portfolio/css` stylesheet and a Storybook story.
- **Squircle corners across the system.** A base rule sets `corner-shape: squircle` on everything, so every rounded corner is a continuous superellipse (the Apple corner) rather than a circular arc. Progressive enhancement — browsers without `corner-shape` fall back to circular rounding, so nothing breaks where it's unsupported.
- **Five motion primitives, reimplemented on the tokens instead of pulled from a library.** The vocabulary a page needs to feel alive without reaching for ReactBits or OriginKit: **ClickSpark** (a ring of rays bursts from a press point, then clears itself — wrap it round a control, it doesn't intercept the click), **BlurReveal** (content resolves from a blur as it rises in, with an optional stagger delay), **StarBorder** (a conic gradient sweeps the border, tinted from `currentColor`, clipped to a thin animated edge), **ShineSweep** (a specular bar travels diagonally across a button or badge) and **LiquidGlass** (an iOS-style frosted surface with a specular highlight that drifts across it). Each pairs a React component (`cx` + the shared `usePrefersReducedMotion`) with a `@paul-portfolio/css` stylesheet, and every one answers `prefers-reduced-motion` — the spark spawns nothing, the reveal is present at rest, the ring stops dead rather than spinning slowly, the sheens hold still; LiquidGlass also answers `prefers-reduced-transparency`, dropping its blur onto a solid surface. Axe-clean (the decorative layers are `aria-hidden`), and Storybook stories cover each. `@paul-portfolio/react` 0.10.4 → 0.11.0, `@paul-portfolio/css` 0.12.4 → 0.13.0.
