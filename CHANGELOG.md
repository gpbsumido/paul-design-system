...
- **BotanicalText, reinterpreted from OriginKit.** Text grown from tiny flowers and leaves that sway. The original renders instanced sprites in Three.js; this samples the text on the built-in 2D canvas and draws little blooms at each point — **no WebGL, no dependency**. A labelled `role="img"`; under reduced motion the sway stops and the blooms are drawn at rest.
